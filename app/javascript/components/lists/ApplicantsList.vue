<template>
  <v-container>
    <v-row v-if="!this.current_user_applicants.length">
      <v-col
    ...
    <v-row v-if="this.current_user_applicants.length">
      <v-col>
        ...
        <v-list
          ...
          width="300"
        >
          <template v-for="applicant in current_user_applicants">
            <v-list-item
              :key="applicant.id"
              link
              @focus="focus(applicant)"
              
            >
              <v-list-item-avatar>
                <v-img
                  v-if="applicant.avatar_url"
                  :src="applicant.avatar_url"
                ></v-img>
                <v-avatar
                  color="primary"
                  size="40"
                  v-if="!applicant.avatar_url && applicant.avatar_name"
                >
                  <span class="white--text text-sm">{{ applicant.avatar_name }}</span>
                </v-avatar>
                <v-avatar
              ...
              <v-list-item-content>
                <v-list-item-title
                  v-text="applicant.name"
                  class="subtitle-1 text--secondary"
                ></v-list-item-title>
                <v-list-item-subtitle
                  v-text="applicant.introduction"
                  class="caption"
                ></v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
...
export default {
  ...
  methods: {
    ...
    focus(applicant) {
      this.both_users = []  // 初期化
      // 指定のプロパティを省いた applicant のサブセット(other) を this.tmp へ返す
        this.tmp = (({ followings, followers, ...other }) => other)(applicant)
      this.tmp_followings_count = applicant.followings.length
      this.tmp_followers_count = applicant.followers.length
      this.tmp_followers = applicant.followers
      // 以下の共通ユーザーを抽出
    ...
    async fetchApplicants() {
      ...
      this.current_user_applicants = res.data.users
