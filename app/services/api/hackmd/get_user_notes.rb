module Api::Hackmd::GetUserNotes
	...
	def get_user_notes(token, id)
		...
		# 	# テスト通信
		# 	sample_id = ids.first
		# 	test_data = Api::Hackmd::Request.get_user_note(token, sample_id).body
		...
		# else
		# 	ids.each do |id|
		# 		res_notes << Api::Hackmd::Request.get_user_note(token, id).body
		# 	end
		# 	# ノートデータの整形
		...
		begin
			# ノートデータの取得
			res_note = Api::Hackmd::Request.get_user_note(token, id).body
			# ノートデータの整形
			user_note_data = res_note.map{ |obj|
				{
					title: obj['title'],
					link: obj['publishLink'],
					short_id: obj['shortId'],
					content: Redcarpet::MdToHtml.change_html(obj['content']),
				} 
			}
			# モデル化
			get_user_note = HackmdNote.new(user_note_data)
